...
export async function proxyScore({ request, env }) {
  ...
  // React hydrates the exact server document. Cloudflare's email obfuscation
  // otherwise rewrites its links and adds a render-blocking decoder script.
  // Preserve the app's existing cache/privacy policy while opting its HTML
  // out of those automatic body transformations; assets and APIs pass through.
  if ((out.get("content-type") || "").toLowerCase().includes("text/html")) {
    const cacheControl = out.get("cache-control") || "";
    if (!/(?:^|,)\s*no-transform\s*(?:,|$)/i.test(cacheControl)) {
      out.set("cache-control", cacheControl ? `${cacheControl}, no-transform` : "no-transform");
    }
  }
